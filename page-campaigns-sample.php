<?php get_header(); ?>
<?php echo get_template_part('template-parts/page-head') ?>

<div class="urgent-campaigns">
    <div class="container">
        <div class="swiper urgent-campaigns-carousel">
            <div class="swiper-wrapper">
                <div class="swiper-slide">
                    <div class="urgent-campaign-img">
                        <img data-src="<?php echo get_template_directory_uri() . '/dist/imgs/urgent-campaign.jpg' ?>" alt="" class="lazyload" loading="lazy">
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-label">
                        <span>Urgent Campaigns</span>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-title">
                        <h2>Provide Food for Syrian Refugees</h2>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-desc">
                        <p>Give the Syrian refugees in camps the food security they need!</p>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-info">
                        <div class="campaign-info--item campaign-time-left">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
                                <path id="Path_209" data-name="Path 209" d="M12,22A10,10,0,1,1,22,12,10,10,0,0,1,12,22Zm0-2a8,8,0,1,0-8-8A8,8,0,0,0,12,20Zm1-8h4v2H11V7h2Z" transform="translate(-2 -2)" fill="#fff" />
                            </svg>

                            <span>30 Days</span>
                        </div>

                        <div class="campaign-info--item campaign-donors">
                            <svg id="Group_269" data-name="Group 269" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                <path id="Path_210" data-name="Path 210" d="M0,0H24V24H0Z" fill="none" />
                                <path id="Path_211" data-name="Path 211" d="M3.161,4.469A6.5,6.5,0,0,1,12,4.141,6.5,6.5,0,0,1,21.179,13.3l-7.765,7.79a2,2,0,0,1-2.719.1l-.11-.1L2.821,13.295a6.5,6.5,0,0,1,.34-8.826ZM4.575,5.883a4.5,4.5,0,0,0-.146,6.21l.146.154L12,19.672l5.3-5.3-3.535-3.535-1.06,1.06A3,3,0,1,1,8.464,7.651l2.1-2.1a4.5,4.5,0,0,0-5.837.189l-.154.146Zm8.486,2.828a1,1,0,0,1,1.414,0l4.242,4.242.708-.706a4.5,4.5,0,0,0-6.211-6.51l-.153.146L9.879,9.065A1,1,0,0,0,9.8,10.392l.078.087a1,1,0,0,0,1.327.078l.087-.078Z" fill="#fff" />
                            </svg>

                            <span>09</span>
                        </div>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-cta">
                        <button class="donation-btn">Donate</button>
                        <a href="#">More</a>
                    </div>
                </div>

                <div class="swiper-slide">
                    <div class="urgent-campaign-img">
                        <img data-src="<?php echo get_template_directory_uri() . '/dist/imgs/urgent-campaign.jpg' ?>" alt="" class="lazyload" loading="lazy">
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-label">
                        <span>Urgent Campaigns</span>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-title">
                        <h2>Provide Food for Syrian Refugees</h2>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-desc">
                        <p>Give the Syrian refugees in camps the food</p>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-info">
                        <div class="campaign-info--item campaign-time-left">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
                                <path id="Path_209" data-name="Path 209" d="M12,22A10,10,0,1,1,22,12,10,10,0,0,1,12,22Zm0-2a8,8,0,1,0-8-8A8,8,0,0,0,12,20Zm1-8h4v2H11V7h2Z" transform="translate(-2 -2)" fill="#fff" />
                            </svg>

                            <span>30 Days</span>
                        </div>

                        <div class="campaign-info--item campaign-donors">
                            <svg id="Group_269" data-name="Group 269" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                <path id="Path_210" data-name="Path 210" d="M0,0H24V24H0Z" fill="none" />
                                <path id="Path_211" data-name="Path 211" d="M3.161,4.469A6.5,6.5,0,0,1,12,4.141,6.5,6.5,0,0,1,21.179,13.3l-7.765,7.79a2,2,0,0,1-2.719.1l-.11-.1L2.821,13.295a6.5,6.5,0,0,1,.34-8.826ZM4.575,5.883a4.5,4.5,0,0,0-.146,6.21l.146.154L12,19.672l5.3-5.3-3.535-3.535-1.06,1.06A3,3,0,1,1,8.464,7.651l2.1-2.1a4.5,4.5,0,0,0-5.837.189l-.154.146Zm8.486,2.828a1,1,0,0,1,1.414,0l4.242,4.242.708-.706a4.5,4.5,0,0,0-6.211-6.51l-.153.146L9.879,9.065A1,1,0,0,0,9.8,10.392l.078.087a1,1,0,0,0,1.327.078l.087-.078Z" fill="#fff" />
                            </svg>

                            <span>09</span>
                        </div>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-cta">
                        <button class="donation-btn">Donate</button>
                        <a href="#">More</a>
                    </div>
                </div>

                <div class="swiper-slide">
                    <div class="urgent-campaign-img">
                        <img data-src="<?php echo get_template_directory_uri() . '/dist/imgs/urgent-campaign.jpg' ?>" alt="" class="lazyload" loading="lazy">
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-label">
                        <span>Urgent Campaigns</span>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-title">
                        <h2>Provide Food for Syrian Refugees</h2>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-desc">
                        <p>Give the Syrian refugees in camps the food security they need!</p>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-info">
                        <div class="campaign-info--item campaign-time-left">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
                                <path id="Path_209" data-name="Path 209" d="M12,22A10,10,0,1,1,22,12,10,10,0,0,1,12,22Zm0-2a8,8,0,1,0-8-8A8,8,0,0,0,12,20Zm1-8h4v2H11V7h2Z" transform="translate(-2 -2)" fill="#fff" />
                            </svg>

                            <span>30 Days</span>
                        </div>

                        <div class="campaign-info--item campaign-donors">
                            <svg id="Group_269" data-name="Group 269" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                <path id="Path_210" data-name="Path 210" d="M0,0H24V24H0Z" fill="none" />
                                <path id="Path_211" data-name="Path 211" d="M3.161,4.469A6.5,6.5,0,0,1,12,4.141,6.5,6.5,0,0,1,21.179,13.3l-7.765,7.79a2,2,0,0,1-2.719.1l-.11-.1L2.821,13.295a6.5,6.5,0,0,1,.34-8.826ZM4.575,5.883a4.5,4.5,0,0,0-.146,6.21l.146.154L12,19.672l5.3-5.3-3.535-3.535-1.06,1.06A3,3,0,1,1,8.464,7.651l2.1-2.1a4.5,4.5,0,0,0-5.837.189l-.154.146Zm8.486,2.828a1,1,0,0,1,1.414,0l4.242,4.242.708-.706a4.5,4.5,0,0,0-6.211-6.51l-.153.146L9.879,9.065A1,1,0,0,0,9.8,10.392l.078.087a1,1,0,0,0,1.327.078l.087-.078Z" fill="#fff" />
                            </svg>

                            <span>09</span>
                        </div>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-cta">
                        <button class="donation-btn">Donate</button>
                        <a href="#">More</a>
                    </div>
                </div>

                <div class="swiper-slide">
                    <div class="urgent-campaign-img">
                        <img data-src="<?php echo get_template_directory_uri() . '/dist/imgs/urgent-campaign.jpg' ?>" alt="" class="lazyload" loading="lazy">
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-label">
                        <span>Urgent Campaigns</span>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-title">
                        <h2>Provide Food for Syrian Refugees</h2>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-desc">
                        <p>Give the Syrian refugees in camps the food security they need!</p>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-info">
                        <div class="campaign-info--item campaign-time-left">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
                                <path id="Path_209" data-name="Path 209" d="M12,22A10,10,0,1,1,22,12,10,10,0,0,1,12,22Zm0-2a8,8,0,1,0-8-8A8,8,0,0,0,12,20Zm1-8h4v2H11V7h2Z" transform="translate(-2 -2)" fill="#fff" />
                            </svg>

                            <span>30 Days</span>
                        </div>

                        <div class="campaign-info--item campaign-donors">
                            <svg id="Group_269" data-name="Group 269" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                <path id="Path_210" data-name="Path 210" d="M0,0H24V24H0Z" fill="none" />
                                <path id="Path_211" data-name="Path 211" d="M3.161,4.469A6.5,6.5,0,0,1,12,4.141,6.5,6.5,0,0,1,21.179,13.3l-7.765,7.79a2,2,0,0,1-2.719.1l-.11-.1L2.821,13.295a6.5,6.5,0,0,1,.34-8.826ZM4.575,5.883a4.5,4.5,0,0,0-.146,6.21l.146.154L12,19.672l5.3-5.3-3.535-3.535-1.06,1.06A3,3,0,1,1,8.464,7.651l2.1-2.1a4.5,4.5,0,0,0-5.837.189l-.154.146Zm8.486,2.828a1,1,0,0,1,1.414,0l4.242,4.242.708-.706a4.5,4.5,0,0,0-6.211-6.51l-.153.146L9.879,9.065A1,1,0,0,0,9.8,10.392l.078.087a1,1,0,0,0,1.327.078l.087-.078Z" fill="#fff" />
                            </svg>

                            <span>09</span>
                        </div>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-cta">
                        <button class="donation-btn">Donate</button>
                        <a href="#">More</a>
                    </div>
                </div>

                <div class="swiper-slide">
                    <div class="urgent-campaign-img">
                        <img data-src="<?php echo get_template_directory_uri() . '/dist/imgs/urgent-campaign.jpg' ?>" alt="" class="lazyload" loading="lazy">
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-label">
                        <span>Urgent Campaigns</span>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-title">
                        <h2>Provide Food for Syrian Refugees</h2>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-desc">
                        <p>Give the Syrian refugees in camps the food security they need!</p>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-info">
                        <div class="campaign-info--item campaign-time-left">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20">
                                <path id="Path_209" data-name="Path 209" d="M12,22A10,10,0,1,1,22,12,10,10,0,0,1,12,22Zm0-2a8,8,0,1,0-8-8A8,8,0,0,0,12,20Zm1-8h4v2H11V7h2Z" transform="translate(-2 -2)" fill="#fff" />
                            </svg>

                            <span>30 Days</span>
                        </div>

                        <div class="campaign-info--item campaign-donors">
                            <svg id="Group_269" data-name="Group 269" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                <path id="Path_210" data-name="Path 210" d="M0,0H24V24H0Z" fill="none" />
                                <path id="Path_211" data-name="Path 211" d="M3.161,4.469A6.5,6.5,0,0,1,12,4.141,6.5,6.5,0,0,1,21.179,13.3l-7.765,7.79a2,2,0,0,1-2.719.1l-.11-.1L2.821,13.295a6.5,6.5,0,0,1,.34-8.826ZM4.575,5.883a4.5,4.5,0,0,0-.146,6.21l.146.154L12,19.672l5.3-5.3-3.535-3.535-1.06,1.06A3,3,0,1,1,8.464,7.651l2.1-2.1a4.5,4.5,0,0,0-5.837.189l-.154.146Zm8.486,2.828a1,1,0,0,1,1.414,0l4.242,4.242.708-.706a4.5,4.5,0,0,0-6.211-6.51l-.153.146L9.879,9.065A1,1,0,0,0,9.8,10.392l.078.087a1,1,0,0,0,1.327.078l.087-.078Z" fill="#fff" />
                            </svg>

                            <span>09</span>
                        </div>
                    </div>

                    <div class="urgent-campaign-item urgent-campaign-cta">
                        <button class="donation-btn">Donate</button>
                        <a href="#">More</a>
                    </div>
                </div>
            </div>

            <div class="autoplay-progress-container">
                <div class="autoplay-progress">
                    <span></span>
                </div>
            </div>
        </div>
    </div>
</div>

<div class="container">
    <!-- Categories & Search -->
    <div class="categories-and-search">
        <div class="categories-filter">
            <a href="" class="category-filter-item">
                <span>Community Health</span>
            </a>
            <a href="" class="category-filter-item">
                <span>Medical Education</span>
            </a>
            <a href="" class="category-filter-item">
                <span>Psycho Support</span>
            </a>
            <a href="" class="category-filter-item">
                <span>Social Support</span>
            </a>
            <a href="" class="category-filter-item">
                <span>Surgical Hospitals</span>
            </a>
            <a href="" class="category-filter-item">
                <span>Training</span>
            </a>
        </div>

        <form method="get" action="<?php echo home_url('/'); ?>" class="search-form">
            <div class="input-holder search-input-holder">
                <div class="search-icon">
                    <svg id="Group_262" data-name="Group 262" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                        <path id="Path_153" data-name="Path 153" d="M0,0H24V24H0Z" fill="none" />
                        <path id="Path_154" data-name="Path 154" d="M18.031,16.617,22.314,20.9,20.9,22.314l-4.282-4.283a9,9,0,1,1,1.414-1.414Zm-2.006-.742a7,7,0,1,0-.15.15l.15-.15Z" fill="#6d54a7" />
                    </svg>
                </div>
                <input type="search" name="s" id="header-search-input" placeholder="<?php _e('Search...', 'bonyan') ?>" class="ps-5 pe-2">
            </div>
        </form>
    </div>

    <div class="cards-container">
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
        <?php get_template_part('template-parts/campaign-card'); ?>
    </div>
</div>

<?php get_footer();
