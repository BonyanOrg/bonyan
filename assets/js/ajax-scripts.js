(function ($) {
  ...
  switch (getLang) {
    ...
    case "ar":
      ...
      generalMsgs = {
        ...
        subscription_cancel_alert_cancel_success_mesg:
    ...
    default: {
      generalMsgs = {
        ...
        subscription_cancel_alert_text:
          "Subscription will be permanently cancelled !",
        ...
        modalBtn.addEventListener("click", function (e) {
          ...
          if (targetedModal !== null) {
            targetedModal.addEventListener("click", function (e) {
              if (
                e.target.classList.contains(targetedModalName) ||
                ...
                document
                  ...
                  .setAttribute("data-qurbandetails", ""); // Reset Qurban Details (for normal donations on guest Mod)
  ...
  $(document).ready(function () {
    ...
    if (donationAction !== null) {
      ...
      document.getElementById("givewp-modal-confirmation").style.display =
        "flex";
      document.getElementById("givewp-modal-confirmation").style.opacity = 1;
    }

    let giveModalConfirmation = document.getElementById(
      "givewp-modal-confirmation"
    );
  ...
  if (fileInput != null) {
    fileInput.addEventListener("change", function () {
      ...
      if (file.length == 1 && file[0].size < 2000000) {
        document.getElementById("user-avatar").src = URL.createObjectURL(
          file[0]
        );
      }
    });
    ...
    $.ajax({
      ...
      statusCode: {
        ...
        200: function (data) {
          ...
          document.querySelector(".user-first-name p").textContent =
            user_FirstName;
          document.querySelector(".user-last-name p").textContent =
            user_LastName;
          document.querySelector(".user-mobile-number p").textContent =
            user_mobile_number;
          document.querySelector(".user-email-address p").textContent =
            user_Email;
          document.querySelector(".user-website p").textContent = user_Website;
          document.querySelector(".user-birth-date p").textContent =
            user_birth_date;
          ...
          document.querySelector(".user-social-facebook a").href =
            user_facebook_url;
          document.querySelector(".user-social-instagram a").href =
            user_instagram_url;
          document.querySelector(".user-social-twitter a").href =
            user_twitter_url;
        },
      },
    });
  ...
  $("#login_form").on("submit", function (e) {
    ...
    grecaptcha.ready(function () {
      grecaptcha
        .execute(ajax_script_object.grecaptcha_site_key, { action: "submit" })
        .then(function (token) {
    ...
    grecaptcha.ready(function () {
      grecaptcha
        ...
        .then(function (token) {
          ...
          if (
            registration_user_password !== registration_user_password_confirm
          ) {
      ...
      $.ajax({
        ...
        statusCode: {
          ...
          200: function (data) {
                ...
                columnDefs: [
                  {
                    ...
                    render: function (data) {
                      ...
                      if (
                        ...
                        data.includes("بالانتظار") ||
                        data.includes("beklemekte")
                      ) {
                        status = "pending";
                      }
                      ...
                      if (
                        data.includes("Abandoned") ||
                        data.includes("غير مكتمل") ||
                        data.includes("eksik")
                      ) {
                        status = "abandon";
                      }
            ...
            if (recurringDataTableInit == true) {
              return;
            }
            $.ajax({
              dataType: "json",
              ...
              statusCode: {
                ...
                200: function (data) {
                  ...
                  $(".recurring-donations-container").append(
                    $(data.HTML_Output)
                  );
                  ...
                  if (recurringDonationTable.length > 0) {
                    ...
                    $("#recurring-donation-table tbody").on(
                      "click",
                      "tr",
                      function () {
  ...
  function addEventListenerToCancelSubscription(recurringDataTable) {
    $(".give-subscription-cancel").on("click", function () {
      ...
      Swal.fire({
        ...
        confirmButtonText:
          generalMsgs.subscription_cancel_alert_confirm_btn_text,
  ...
  $("#charity_select").on("change", function () {
    ...
    $("#quick_donate_now_btn").attr("data-tagName", ""); // reset
    $("#quick_donate_now_btn").attr(
      "data-tagName",
      $("#charity_select option:selected").text()
    ); // set new value
    $.ajax({
      dataType: "json",
      method: "POST",
    ...
    if (ajaxSearchInput !== null) {
      ajaxSearchInput.addEventListener("input", function () {
        ...
        $(".cards-container").append(
          '<div class="loader" style="display: flex;"></div>'
        );
        ...
        keyPressTimeout = setTimeout(function () {
          jqxhr = $.ajax({
            ...
            statusCode: {
              400: function (data) {
                $(".cards-container").append(
                  `<p style="font-size: 20px;" class="primary-color">${data.responseJSON.error_message}</p>`
                );
                $(".loader").remove();
              },
              200: function (data) {
      ...
      $.ajax({
        ...
        statusCode: {
          ...
          200: function (data) {
            ...
            let DonationHistoryTable = $(
              "#donation-history-table-in-history-tab"
            );
  ...
  $("#recurring-donations-tab-btn").on("click", function () {
    if (modern_DontionReacurring_isOpened) {
      return;
    }
      ...
      statusCode: {
        ...
        200: function (data) {
          ...
          if (recurringDonationTable.length > 0) {
            ...
            $("#recurring-donations-table tbody").on(
              "click",
              "tr",
              function () {
                // For mobile And Responsive
